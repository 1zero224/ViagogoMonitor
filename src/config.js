...
function loadConfig(argv = []) {
  ...
  return {
    ...
    listingProgressTimeoutMs: Math.max(1000, parseNumber(process.env.LISTING_PROGRESS_TIMEOUT_MS, 12000)),
    listingStableWindowMs: Math.max(250, parseNumber(process.env.LISTING_STABLE_WINDOW_MS, 1500)),
    listingFinalSettleTimeoutMs: Math.max(1000, parseNumber(process.env.LISTING_FINAL_SETTLE_TIMEOUT_MS, 5000)),
    ...
    scraperUserAgent: stripWrappingQuotes(process.env.SCRAPER_USER_AGENT) || null,
    debounceListingAvailabilityAlerts: parseBoolean(process.env.DEBOUNCE_LISTING_AVAILABILITY_ALERTS, true),
    listingAvailabilityConfirmRuns: Math.max(2, parseNumber(process.env.LISTING_AVAILABILITY_CONFIRM_RUNS, 2)),
