...
### 8.2 Railway Cron Schedule 配置

当前 Railway 的定时任务入口在**服务自己的设置页**，不是单独再创建一个 `Cron` 服务。
...
- Railway 会按这个计划直接执行当前服务的启动命令
- 当前仓库是一次性 runner，正适合这种模式
- 不需要再在代码里套 `node-cron`
- 不需要单独创建新的 Web 服务来调自己
- 这个服务运行完应该自然退出，否则下一次计划任务会被跳过
...
- Railway Cron 的最短间隔不能小于 5 分钟
- Railway Cron 使用的是 **UTC** 时区，不是北京时间
...
- 北京时间每天上午 10:00
  - 要写成 `0 2 * * *`
  - 因为北京时间 UTC+8，对应 UTC 的 02:00
- 北京时间每天晚上 20:30
  - 要写成 `30 12 * * *`
  - 因为北京时间 UTC+8，对应 UTC 的 12:30
